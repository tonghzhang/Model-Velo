...
| 阶段 | 完成/总数 | 状态 | 说明 |
|---|---:|---:|---|
| S0 项目治理与基线 | 12/15 | 进行中 | Git、忽略规则、README、架构现状和密钥基线已同步，治理模板仍待补齐 |
...
| Chat Provider 协议补强 | 6/6 | 已完成 | Tool、结构化输出、多模态、原生流和能力声明已完成合并协议测试 |
| S6 工程化与求职展示 | 44/75 | 进行中 | 可观测性、CI 配置、基础 benchmark、容器与运维文档已落地；race、远端 CI、完整性能矩阵和最终演示仍未完成 |
| **合计** | **337/383** | **88.0%** | 百分比只表示任务数量，不表示风险或工期完成度 |
...
- [x] `S0-006` **初始化 Git 仓库** — 实施：确认仓库根目录后执行 Git 初始化并设置主分支；完成证据：`git status` 可用且只显示预期文件，不自动提交。
- [x] `S0-007` **建立 `.gitignore`** — 实施：排除 `.env`、二进制、覆盖率、benchmark 临时文件、IDE、本地数据库与运行日志；完成证据：密钥和构建产物不会进入 `git status`。
...
- [ ] `S5-045` **完成阶段 5 门禁并等待用户确认** — 当前：用户已允许进入 S6，Usage v2 真实 Redis/PostgreSQL 集成、故障测试、全量测试、vet、文档和独立性复查已完成；Worker race 仍在 `runtime/cgo` 构建阶段被本机工具链阻止，因此阶段 5 的完整门禁继续保留为未通过。
...
### 8.6 Chat Provider 协议完整性补强

- [x] `P-001` **建模 Function Tool 往返** — 实施：统一解析 tools、tool choice、assistant tool calls 与 tool result 引用；完成证据：重复 ID、未知引用和非对象参数在上游调用前失败。
- [x] `P-002` **补齐原生 Tool 转换** — 实施：Anthropic、Gemini、DashScope、Cohere、Ollama、Bedrock 映射定义、历史、结果和响应；完成证据：合并假上游矩阵返回 OpenAI `tool_calls` 与 `finish_reason=tool_calls`。
- [x] `P-003` **补齐结构化输出** — 实施：映射 JSON object/schema，并对 DashScope schema、Cohere tools+format 等无等价组合明确拒绝；完成证据：请求体断言覆盖六种原生协议。
- [x] `P-004` **补齐多模态内容模型** — 实施：识别 image、audio、file 及 detail，按协议转换 URL、Base64、inlineData、document；完成证据：Gemini 音频/文件、Anthropic/Bedrock 文件、Cohere 图片和既有 VLM 矩阵通过。
- [x] `P-005` **补齐所有内置 Chat 流** — 实施：转换四种原生 SSE、Ollama NDJSON、Bedrock EventStream，并给 Azure/Cloudflare 接入兼容 SSE；完成证据：文本、Tool 参数增量、finish、usage 和 `[DONE]` 合并测试通过。
- [x] `P-006` **让能力声明与实现一致** — 实施：路由能力扩为 text/image/audio/file/tools/structured，原生不等价字段不再静默删除；完成证据：能力推导和协议上限测试通过。

## 9. S6：可观测性、CI、Benchmark、故障测试与求职展示

### 9.1 结构化日志与运行诊断

- [x] `S6-001` **确认阶段 6 交付范围** — 实施：工程化已有能力，不借机新增未经验证的大功能；完成证据：用户明确允许进入 S6。
- [x] `S6-002` **选择并统一结构化日志方案** — 实施：定义 JSON/文本开发模式、级别和时间格式；完成证据：API/Worker 不混用无结构输出。
- [x] `S6-003` **定义请求公共日志字段** — 实施：request ID、tenant ID、模型、方法、路径、状态、耗时；完成证据：字段名稳定且 tenant 不从 Key 推断输出。
- [x] `S6-004` **实现敏感信息脱敏规则** — 实施：过滤 Authorization、Provider Key、DSN 密码、Cookie、提示词和完整响应；完成证据：单元测试扫描日志内容。
- [x] `S6-005` **记录请求生命周期日志** — 实施：请求结束时输出一次摘要，避免每层重复噪声；完成证据：成功、缓存、失败和取消可区分。
- [x] `S6-006` **记录 Provider Attempt 日志** — 实施：只记录 Provider/Key ID、attempt、类别、耗时和结果；完成证据：Retry/Fallback 顺序可重建但无 Secret。
...
- [x] `S6-009` **建立日志行为测试** — 实施：捕获输出验证字段、级别、request ID 和脱敏；完成证据：构造的假 Key/DSN/提示词不出现在日志。

### 9.2 Prometheus 指标

- [x] `S6-010` **设计有界指标标签** — 实施：只使用状态、错误类别、Provider ID、模型等受控集合；完成证据：禁止 request ID、tenant ID、原始 URL/Key 作为 label。
- [x] `S6-011` **暴露 API `/metrics`** — 实施：使用独立 registry/Handler 并明确是否需要访问保护；完成证据：抓取不经过聊天鉴权链路。
- [x] `S6-012` **统计请求总数** — 实施：按路由、结果和流式模式计数；完成证据：测试请求使计数按预期增长。
- [x] `S6-013` **统计请求延迟分布** — 实施：设置适合 LLM 请求的 histogram buckets；完成证据：单位固定且缓存/非缓存可区分。
- [x] `S6-014` **统计当前在途请求** — 实施：进入增加、所有结束路径减少；完成证据：取消/panic 后 gauge 回零。
- [x] `S6-015` **统计 Provider Attempt 与延迟** — 实施：按 Provider、结果、错误类别观测；完成证据：Retry 每次 attempt 可见。
- [x] `S6-016` **统计 Retry 与 Fallback** — 实施：分别记录原因和结果；完成证据：一次请求的 retry/fallback 数与测试场景一致。
...
- [x] `S6-019` **统计 Auth、Rate Limit 与 Cache** — 实施：认证结果、限流拒绝、cache hit/miss/error/bypass；完成证据：故障降级路径有独立指标。
- [x] `S6-020` **统计 Usage 数据链路** — 实施：XADD、消费、DB 写入、重复、pending、claim、dead-letter；完成证据：Worker 故障测试反映到指标。
- [x] `S6-021` **建立指标单元测试** — 实施：使用测试 registry 收集并断言值/标签；完成证据：不会依赖全局 registry 导致测试污染。

### 9.3 必要的 Tracing

- [x] `S6-022` **选择 OpenTelemetry 依赖与最小配置** — 实施：配置 service name、resource、sampler 和 exporter；完成证据：禁用 exporter 时应用仍可运行。
- [x] `S6-023` **创建 HTTP 根 Span** — 实施：从入站请求提取 trace context 并关联 request ID；完成证据：状态和错误类别被记录。
- [ ] `S6-024` **为关键内部阶段创建子 Span** — 实施：Auth、Rate Limit、Cache、Route、Queue wait、Attempt；完成证据：不过度为普通函数创建碎片 Span。
- [x] `S6-025` **为上游 Provider 创建 Client Span** — 实施：记录安全 URL 元数据、Provider 和状态；完成证据：不记录 Authorization 或请求 Body。
- [x] `S6-026` **传播 Trace Context 到上游** — 实施：按标准 Header 注入且不信任任意客户端敏感 Header；完成证据：假上游能观察同一 trace。
- [ ] `S6-027` **追踪 Redis/PostgreSQL 关键调用** — 实施：限流、Cache、XADD、Worker 写库使用必要 Span；完成证据：不会产生高基数 SQL/Key 属性。
- [x] `S6-028` **处理流式 Span 生命周期** — 实施：Span 在流真正完成/中断时结束，而不是 Header 提交时；完成证据：duration 覆盖完整 SSE。
- [ ] `S6-029` **建立 Trace 测试** — 实施：使用内存 exporter 断言父子关系、错误状态和脱敏；完成证据：测试不依赖外部 Collector。

### 9.4 GitHub Actions 与质量门禁

- [x] `S6-030` **创建基础 CI Workflow** — 实施：在 push/PR 上使用固定 Go 版本和依赖缓存；完成证据：空缓存环境可运行。
- [x] `S6-031` **在 CI 检查 gofmt** — 实施：格式不一致直接失败并显示文件；完成证据：不在 CI 静默改写源码。
- [x] `S6-032` **在 CI 运行全部单元测试** — 实施：`go test ./...` 不访问付费公网 API；完成证据：假 Provider 覆盖上游。
- [x] `S6-033` **在 CI 运行 `go vet`** — 实施：静态问题使 Job 失败；完成证据：命令和本地一致。
- [ ] `S6-034` **在 CI 运行 race 测试** — 实施：覆盖 Breaker、Queue、Key、SSE、Worker 并发包；完成证据：不因耗时随意跳过核心并发测试。
- [x] `S6-035` **在 CI 提供 Redis/PostgreSQL 服务** — 实施：使用固定版本和健康等待；完成证据：集成测试不依赖开发者机器。
- [x] `S6-036` **隔离并清理集成测试数据** — 实施：每次 Run 使用独立 DB/schema/key namespace；完成证据：并行或重跑不互相污染。
- [x] `S6-037` **在 CI 验证 GORM Schema** — 实施：执行 AutoMigrate 幂等检查和数据库约束测试；完成证据：模型与真实 schema 不一致时失败。
...
- [x] `S6-043` **定义标准请求载荷** — 实施：记录模型、消息/工具大小、响应 token/chunk 模式；完成证据：报告可重放相同负载。
...
- [x] `S6-052` **建立 Redis 故障测试** — 实施：停机、超时、连接耗尽，验证 limiter/cache/emitter 各自策略；完成证据：行为与配置文档一致。
- [ ] `S6-053` **建立 PostgreSQL 故障测试** — 实施：停机、慢查询、连接池耗尽和事务失败；完成证据：Auth/Worker 不无限等待或错误 ACK。
- [x] `S6-054` **建立 Provider 故障测试** — 实施：DNS/连接拒绝、5xx、429、慢首包、断流；完成证据：Retry/Breaker/Fallback/SSE 行为符合矩阵。
- [x] `S6-055` **形成可复现报告** — 实施：提交命令、配置、原始摘要和结论限制；完成证据：不使用“高并发/生产可用”空泛结论。
...
- [x] `S6-059` **完成 README Quickstart** — 实施：从依赖启动、GORM schema 同步、Key 创建到 curl 请求；完成证据：全新环境能复现成功链路。
- [x] `S6-060` **完成配置参考** — 实施：列出每个变量、默认值、必填性、安全说明和阶段；完成证据：实现与文档自动/人工核对。
- [x] `S6-061` **完成 API 与 SDK 示例** — 实施：提供 curl 和至少一种 OpenAI SDK 的非流式/SSE 示例；完成证据：示例可运行且使用假 Key。
- [x] `S6-062` **完成错误与恢复策略文档** — 实施：说明 400、401/403、429、5xx、网络、取消、Retry/Fallback/Breaker；完成证据：与测试矩阵一致。
- [x] `S6-063` **完成 Auth、Rate Limit 与 Cache 文档** — 实施：描述租户隔离、Key 哈希、限流故障策略和 Cache Key/TTL；完成证据：不泄露实现密钥。
- [ ] `S6-064` **完成 SSE 首 Chunk 文档** — 实施：说明提交前后 Retry/Fallback 边界与取消；完成证据：配有可运行故障演示。
- [x] `S6-065` **完成 Usage 可靠性文档** — 实施：画出 XADD→Group→幂等 DB→XACK，说明 pending/毒消息；完成证据：明确 at-least-once。
- [x] `S6-066` **更新架构图为已实现状态** — 实施：区分真实包/进程与概念模块；完成证据：图中不存在未实现却未标注的能力。
- [x] `S6-067` **记录关键架构决策** — 实施：解释单体多进程、无 DDD/插件平台、Queue/Breaker/Fallback 边界等；完成证据：每项包含取舍和后果。
...
- [x] `S6-071` **执行最终字面重复度检查** — 实施：分别对 GoModel、Bifrost、合并集合运行固定工具/参数；完成证据：报告工具版本、阈值、结果区间且严格小于 10%。
- [x] `S6-072` **执行最终归一化近似检查** — 实施：去空白/局部标识符差异后按 12 行或 80 token 阈值检测；完成证据：三种参考口径均严格小于 10%。
- [x] `S6-073` **执行最终人工独立性复核** — 实施：检查控制流、独特命名、注释和错误字符串；完成证据：发现相似项通过重新设计解决而非稀释分母。
